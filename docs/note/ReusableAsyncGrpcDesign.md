...
## Should We Use C++20 Coroutines?

Short answer:

- yes, coroutine is available in this repository
- no, it should not be the primary abstraction for this refactor

### Why it is technically available

TiFlash already builds with modern C++, so language-level coroutine support is not the blocker.

Also, the bundled gRPC already contains callback/reactor APIs for:
...
So if we want to experiment with coroutine-based wrappers in the future, the toolchain is capable of supporting it.

### Why it should not be the main abstraction here

The hard part of TiFlash async gRPC today is not mainly “state machines are ugly”. The hard part is the set of concurrency constraints below:

1. One stream direction allows only one outstanding `Read` or `Write` at a time.
2. Business threads and CQ poller threads must communicate without lost notifications.
3. Some RPCs need external waiting, for example waiting for an MPP tunnel or task to become visible.
4. Shutdown and self-destruction rules are delicate because CQ events may arrive on different poller threads.

Coroutine does not remove any of those constraints. It only changes how they are expressed.

If we directly build coroutine awaiters on top of the current raw CQ/tag model, we are likely to just move complexity from:

- handwritten `switch(state)`

to:

- coroutine promise type
- awaiter ownership
- suspension/resume scheduling
- cancellation semantics
- coroutine frame lifetime

That is not a clear net win for the first refactor.

### Where coroutine may still be useful

Coroutine can still be a good fit as an upper-layer expression tool after the lower-level async framework is stabilized.

Good candidates:

- unary RPCs
- simple one-direction streaming flows
- business-level waits such as “wait until resource ready”

Less suitable as the first coroutine migration target:

- `EstablishMPPConnection`
- bidirectional streaming
- any RPC whose core difficulty is cross-thread wake-up and lifecycle ownership

### Recommended stance

For this design, coroutine should be treated as optional sugar above the reusable async foundation, not as the foundation itself.

In practice that means:

1. First extract typed tags, flow bridges, direction pumps, and call lifecycle.
2. Keep CQ/tag ownership explicit in the core layer.
3. Only after that, consider coroutine adapters that `co_await`:
   - next inbound message
   - next outbound slot
   - external resource readiness
   - final RPC completion

This keeps the hot path and lifecycle model close to the current proven implementation, while still leaving room for coroutine-based business code later.
